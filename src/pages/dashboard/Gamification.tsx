...
const challenges = [
  { key: 'saveEnergy', icon: Zap, progress: 78, target: 100, color: 'text-gold', bg: 'bg-gold-light' },
  { key: 'reduceWater', icon: Droplets, progress: 62, target: 100, color: 'text-blue-500', bg: 'bg-blue-100 dark:bg-blue-900/30' },
  { key: 'recyclingChallenge', icon: Recycle, progress: 45, target: 100, color: 'text-palm-500', bg: 'bg-palm-100 dark:bg-palm-900/30' },
];

const leaderboard = [
  { rank: 1, name: 'Class 10-A', score: 2450, icon: Crown, color: 'text-gold' },
...
const badges = [
  { key: 'ecoHero', icon: Award, color: 'text-palm-500', bg: 'bg-palm-100 dark:bg-palm-900/30', earned: true },
  { key: 'greenLeader', icon: Crown, color: 'text-saudi-green', bg: 'bg-palm-100 dark:bg-palm-900/30', earned: true },
  { key: 'energyGuardian', icon: Zap, color: 'text-gold', bg: 'bg-gold-light', earned: false },
...
export default function Gamification() {
  ...
  return (
    <motion.div
      ...
      className="space-y-6"
    >
      <h2 className="text-2xl font-bold font-display">{t('gamificationNav', lang)}</h2>
          ...
          {challenges.map((challenge) => (
            <div key={challenge.key}>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-bold text-saudi-green">
              ...
              <div className="w-full h-3 rounded-full bg-[var(--bg-secondary)] overflow-hidden">
                <motion.div
                  ...
                  className="h-full rounded-full bg-gradient-to-r from-saudi-green to-palm-500"
                />
              </div>
            </div>
          ))}
          ...
          <div className="grid grid-cols-2 gap-4">
            {badges.map((badge) => (
              ...
              >
                ...
                {badge.earned && (
                  <span className="text-xs px-2 py-0.5 rounded-full bg-palm-100 dark:bg-palm-900/30 text-palm-600 dark:text-palm-400 font-medium">
